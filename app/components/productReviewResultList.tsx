...
const ProductReviewResultList: React.FC<ProductReviewResultListProps> = ({ ratings }) => {
  ...
  if (!ratings?.length) {
    return <EmptyState
      messageKey="info.review.message_01"
      fallbackMessage="Review is empty."
        ...
        {ratings.map((rating) => {
          ...
          const helpfulOptions = [
            {
              id: '0',
              value: 'yes',
              label: t('info.review.message_03'),
              icon: (
                <motion.div
                  key={selectedValue === 'yes' ? 'yes-active' : 'yes-inactive'}
                  whileTap={{ scale: 1.3 }}
                  animate={{ scale: selectedValue === 'yes' ? 1.2 : 1 }}
            ...
            },
            {
              id: '1',
              value: 'no',
              label: t('info.review.message_04'),
              icon: (
                <motion.div
                  key={selectedValue === 'no' ? 'no-active' : 'no-inactive'}
                  whileTap={{ scale: 1.3 }}
                  animate={{ scale: selectedValue === 'no' ? 1.2 : 1 }}
                  transition={{ type: 'spring', stiffness: 300 }}
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
          ...
          return (
            <div key={id} className="gap-3 py-6 sm:flex sm:items-start">
              <div className="shrink-0 space-y-2 sm:w-48 md:w-62">
                ...
                <div className="inline-flex items-center gap-1">
                  <svg className="h-5 w-5 text-green-900 dark:text-primary-500" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
                    <path
                      fillRule="evenodd"
