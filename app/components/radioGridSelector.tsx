    ...
    >
      ...
      {options.map((option) => (
        <div key={option.id}>
          <label
            htmlFor={option.id}
            className={`block cursor-pointer rounded-lg border border-gray-100 bg-white p-4 text-sm font-medium shadow-sm hover:border-gray-200 ${
              String(selectedValue) === String(option.value) ? 'border-blue-500 ring-1 ring-blue-500' : ''
            }`}
          >
            <div>
